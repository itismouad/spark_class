...
custBaskets = parsedLines.map(lambda x: (x[0], x[2])).reduceByKey(lambda x, y: x + y)
flipped = custBaskets.map(lambda x: (x[1], x[0])).sortByKey()

# collect and display results
results = flipped.collect();

for result in results:
    print("$" + str(round(result[0], 2)) + " for customerID = " + str(result[1]))
